...
BLDGS="https://services.arcgis.com/bDAhvQYMG4WL8O5o/ArcGIS/rest/services/buildingfootprint_2075_USACE_2mBuff_noWall/FeatureServer/77/query"
AERIAL="https://geo.vbgov.com/imageservices/rest/services/Imagery/Aerial2025/ImageServer/exportImage"

def gj(u,p):
    q=urllib.parse.urlencode(p)
    req=urllib.request.Request(u+"?"+q,headers={"User-Agent":"HVAC-Territory/0.7.4"})
...
def bbox(x,y,m):
    dy=m/69
    dx=m/(69*max(.2,math.cos(math.radians(y))))
    return x-dx,y-dy,x+dx,y+dy

def miles(x,y,a,b):
    return math.hypot((a-x)*69.172*math.cos(math.radians(y)),(b-y)*69)
...
def pages(u,p):
    out=[];off=0
    while True:
        q=dict(p);q["resultOffset"]=off;q["resultRecordCount"]=1800
        d=gj(u,q);fs=d.get("features",[])
        out.extend(fs)
        if not d.get("exceededTransferLimit") and len(fs)<1800:break
        if not fs:break
        off+=len(fs)
    return out
...
def load_parcels(x,y,mi):
    ...
    for f in pages(PARCEL,p):
        at=f.get("attributes",{});rs=f.get("geometry",{}).get("rings",[])
        cx,cy=centroid(rs)
...
def load_buildings(x,y,mi):
    ...
    for f in pages(BLDGS,p):
        ...
        if cx is not None and miles(x,y,cx,cy)<=mi*1.05:
            out.append({"lon":cx,"lat":cy,"sq":round(area(rs))})
    return out

def classify(land,zone,largest,avg,count):
    s=(land+" "+zone).upper()
    high=("HOSP","MEDICAL","UNIVERS","COLLEGE","INDUSTR","MANUFACTUR","GOVERN","SCHOOL",
          "WAREHOUSE","DISTRIBUT","UTILITY","PUBLIC/SEMI PUBLIC")
    ...
    if largest:
        score+=10 if largest>=75000 else 6 if largest>=40000 else 3 if largest>=20000 else 0
    # Many-small-building penalty: catches housing/townhome-style campuses.
...
def discover(x,y,mi,mn):
    ...
    for b in bs:
        ...
        if hits:
            min(hits,key=lambda p:p["psq"] or 1e99).setdefault("bs",[]).append(b)
    rows=[]
    for p in ps:
        bl=p.get("bs",[])
        largest=max((b["sq"] for b in bl),default=None)
        total=sum(b["sq"] for b in bl) if bl else None
        count=len(bl);avg=round(total/count) if count else None
        ...
        p.update(largest=largest,total=total,count=count,avg=avg,
                 distance=round(miles(x,y,p["lon"],p["lat"]),2))
        p["tier"],p["score"]=classify(p["land"],p["zone"],largest,avg,count)
        p["path"]="SIZE" if sizepass else "ANOMALY"
        p["source"]="VGIN LOCAL" if bl else "FOOTPRINT MISSING"
        rows.append(p)
    ded={}
    for p in rows:
...
def aerial(x,y,sf,out):
    ...
    q=urllib.parse.urlencode({"f":"image","bbox":f"{X-h},{Y-h},{X+h},{Y+h}",
                              "bboxSR":"3857","imageSR":"3857","size":"1800,1800","format":"jpg"})
    with urllib.request.urlopen(AERIAL+"?"+q,timeout=120) as r:Path(out).write_bytes(r.read())

class App:
    def __init__(self,r):
        self.r=r;self.rows=[]
        r.title("HVAC Territory Discovery v0.7.4 — Local Footprints + Two Paths")
        r.geometry("1560x850")
        ...
        ttk.Entry(t,textvariable=self.q,width=40).grid(row=0,column=1,padx=5)
        ttk.Label(t,text="Radius mi:").grid(row=0,column=2)
        self.rad=tk.StringVar(value="1.0");ttk.Entry(t,textvariable=self.rad,width=6).grid(row=0,column=3)
        ttk.Label(t,text="Size path min ft²:").grid(row=0,column=4)
        self.mn=tk.StringVar(value="10000");ttk.Entry(t,textvariable=self.mn,width=8).grid(row=0,column=5)
        self.b=ttk.Button(t,text="Discover",command=self.start);self.b.grid(row=0,column=6,padx=8)
        self.st=tk.StringVar(value="SIZE = meets threshold. ANOMALY = smaller non-residential site retained for future mechanical screening.")
        ttk.Label(r,textvariable=self.st).pack(fill="x",padx=10)
        cs=("rank","address","largest","total","avg","bldgs","miles","land","zone","tier","score","path","source")
        self.tree=ttk.Treeview(r,columns=cs,show="headings")
        for c,w in zip(cs,(45,240,85,90,80,55,60,205,60,75,55,75,105)):
    ...
    def start(self):
        ...
        self.st.set("Loading city parcels + VGIN/VBMP locality-submitted footprints...")
        threading.Thread(target=self.work,daemon=True).start()
    def work(self):
        try:
            x,y=geocode(self.q.get().strip())
            self.rows=discover(x,y,float(self.rad.get()),float(self.mn.get()))
    ...
    def show(self):
        ...
        for n,z in enumerate(self.rows,1):
            ...
            self.tree.insert("","end",iid=str(n-1),values=(n,z["address"],fmt(z["largest"]),fmt(z["total"]),
                fmt(z["avg"]),z["count"],z["distance"],z["land"],z["zone"],z["tier"],z["score"],z["path"],z["source"]))
        sz=sum(z["path"]=="SIZE" for z in self.rows)
        self.st.set(f"{len(self.rows)} prospects — SIZE {sz}, ANOMALY {len(self.rows)-sz}. Many-small-building penalty active.")
        self.b.config(state="normal")
    def fail(self,e):
        self.st.set("Failed: "+repr(e));self.b.config(state="normal")
    def sel(self):
    ...
    def dl(self):
        z=self.sel()
        if not z:return
        out=Path.home()/"Downloads"/f'HVAC_{z["address"].replace(" ","_") or "candidate"}.jpg'
