...
def ask(c,prompt,content,schema,name,tok=2200):
 r=c.responses.create(model=MODEL,reasoning={"effort":"medium"},input=[{"role":"user","content":[{"type":"input_text","text":prompt}]+content}],text={"format":{"type":"json_schema","name":name,"strict":True,"schema":schema},"verbosity":"low"},max_output_tokens=tok)
 if r.status!="completed":raise RuntimeError(f"response {r.status}: {getattr(r,'incomplete_details',None)}")
...
class A:
 ...
 def show(self,x,o,u):
  ...
  self.s.set("10-view inspection complete")
